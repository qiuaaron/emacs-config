...
(defun xref-add-optional-pars-to-make (mcommand)
  (let ((res))
	(if (equal (substring mcommand 0 5) "make ")
		(setq res (format "make -i %s" (substring mcommand 5)))
	  (setq res mcommand)
	  )
	res
))
		...
		(progn
		  ...
		  (if recipe
			  (progn
				...
				(insert (format "  -set buildrecipe \"\n\tcd %s\n\t%s\n\t%s %s %s\n\t\"\n" prbdirectory prccommand (xref-get-full-executable-name "xref-recipe-build") recipe (xref-add-optional-pars-to-make prbcommand)))
				))
	...
	(if hasrecipe
		(progn
		  (setq prbcommand (read-from-minibuffer 
							"Command to build the project: " prbcommand))
		  (setq prccommand (read-from-minibuffer 
							"Command to clean the project: " prccommand ))
...
(defun xref-create-batch-file (bdir command)
  (let ((res) (cc))
	...
	(setq cc (format "%scd %s\n%s\n" cc (xref-backslashify-name (xref-remove-starting-tilda bdir)) command))
